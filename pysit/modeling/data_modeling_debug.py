...
#       print "  Computing step {0}...".format(k)
        ...
        if k == 0:
            ...
            rhs_kp1 = mesh.pad_array(source.f((k+1)*dt), out_array=rhs_kp1)
        tt_6 = tt_6 + (time.time() - tt_tmp)

        # Given the state at k and k-1, compute the state at k+1
        tt_tmp = time.time()
        solver.time_step(solver_data, rhs_k, rhs_kp1)
        tt_7 = tt_7 + (time.time() - tt_tmp)
        # Don't know what data is needed for the solver, so the solver data
        # handles advancing everything forward by one time step.
        # k-1 <-- k, k <-- k+1, etc
        tt_tmp = time.time()
        solver_data.advance()
        tt_8 = tt_8 + (time.time() - tt_tmp)

    print('Time 1 time is', tt_1)
    print('Time 2 time is', tt_2)
    print('Time 3 time is', tt_3)
    print('Time 4 time is', tt_4)
    print('Time 5 time is', tt_5)
    print('Time 6 time is', tt_6)
    print('Time 7 time is', tt_7)
    print('Time 8 time is', tt_8)
